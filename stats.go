...
func stats(email, path string) {
	commits := processRepository(email, path)
...
// processRepository given an user email, returns the
// commits made in the last 6 months
func processRepository(email, path string) map[int]int {
